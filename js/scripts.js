var user_info = [
  {
    ...
    password: "123456789",
    state: 0,
    phoneNumber: "088978888",
    address: "DaLat",
    email: "nhom08pro@gmail.com",
    isBlocked: 0,
    resetPW: 0,
  },
];
...
if (JSON.parse(localStorage.getItem("cart")) != null) {
  ...
  if (cartCount1 != 0) {
    document.querySelector(".cart-btn .after").style.display = "block";
    document.querySelector(".cart-btn .after").innerHTML = cartCount1;
  } else {
    document.querySelector(".cart-btn .after").style.display = "none";
  }
} else {
  document.querySelector(".cart-btn .after").style.display = "none";
}
...
function addToCart(id) {
  ...
  if (document.getElementById("userName") == null)
    ...
    var userName = document.getElementById("userName").text;
    if (isBlocked(userName) == 1) alert("Tài khoản của bạn đã bị khóa, vui lòng liên hệ admin page để giải quyết!");
    else {
...
function quantitydown2(id) {
  ...
  for (var i = 0; i < stored_cart.length; i++) {
    if (id.slice(0, -1) == stored_cart[i].id) {
      if (stored_cart[i].quantity > 1) stored_cart[i].quantity--;
    }
  }
  localStorage.setItem("cart", JSON.stringify(stored_cart));
  showCart();
    ...
    if (userName == stored_order[i].name) {
      ...
      if (checkIdClient(stored_order[i].id, id_client_order) == false)
        id_client_order.push({
          ...
          phoneNumber: stored_order[i].phoneNumber,
...
const orderAddressBtn = () => {
  if (
    ...
    alert("Nhớ nhập đầy đủ thông tin của địa chỉ mới nha.");
    ...
    var address = `Số nhà ${houseNumber} đường ${street} phường ${ward} quận ${district} thành phố ${city}`;
    var stored_cart;
    ...
    for (let i = 0; i < userInfo.length; i++) {
      if (document.querySelector("#userName").innerHTML == userInfo[i].name) {
        phoneNumber = userInfo[i].phoneNumber;
      }
    }
    for (var i = 0; i < stored_cart.length; i++) {
...
function sendOrder() {
  ...
  var userName = document.getElementById("userName").text;
  var stored_cart = JSON.parse(localStorage.getItem("cart"));
  let userConfirm = confirm("Bạn có muốn cập nhật địa chỉ mới hay không ?");
  if (userConfirm) {

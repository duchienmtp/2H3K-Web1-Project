    ...
    if (save_order[i].id == OrderID)
      for (var j = 0; j < save_order[i].order.length; j++)
        total_price += parseInt(
          getPrice(save_order[i].order[j][2], save_order[i].order[j][0])
        );
  return total_price.toLocaleString("it-IT", {
    style: "currency",
    currency: "VND",
  });
}
...
function showDetail(id) {
  ...
  for (var i = 0; i < save_order.length; i++) {
    if (id == save_order[i].id) {
      temp +=
        '<li style="margin-bottom: 1.5%; list-style-type: none;"><p>Name: ' +
        save_order[i].name +
        "</p><p>Order ID: " +
        save_order[i].id +
        "</p>" +
      ...
      temp += "</ul></li>";
      temp +=
        '<button class="detail" id="' +
        save_order[i].id +
        '" onclick="changeState(this.id)"style="margin-left: 7%">Process</button>';
      break;
    }
  }
  ...
  if (localStorage["save_order"] != null) {
    ...
    for (var i = save_order.length - 1; i >= 0; i--) {
      result +=
        ...
        '<button class="detail" id="' +
        save_order[i].id +
        '" onclick="showDetail(this.id)" style="margin-bottom: 5px">Detail</button>' +
        "</li>";
    }
...
function createPage(product_info) {
  ...
  for (var i = 0; i < getNum(product_info); i++) {
    ...
    for (var j = 12 * i; j < 12 * i + 12; j++) {
      if (product_info[j] != null) {
        temp +=
          ...
          '<img src="' +
          product_info[j].img +
          '"><h4 style="font-size: 20px; margin-top:2%">' +
          product_info[j].name +
          '</h4><div class="rating"><span style="color: black">&#9733</span><span style="color: black">&#9733</span><span style="color: black">&#9733</span><span style="color: black">&#9733</span><span style="color: black">&#9733</span></div><p>Price: ' +
          product_info[j].price.toLocaleString("it-IT", {
            style: "currency",
            currency: "VND",
          }) +
          '</p><p style="margin-top: -15px">Sold: ' +
          (100 - product_info[j].quantity) +
          '</p><p style="margin-top: -15px">In stock: ' +
          product_info[j].quantity +
          "</p>" +
          "</div>";
        c += 1;
    ...
    products =
      '<h2 class="title">' + "SALE REPORT/" + (i + 1) + "</h2>" + products;
...
function changePage(id) {
  ...
  if (parseInt(id) == 0) {
    ...
    result =
      ...
      '<div id="monthOfChart"><p style="display: inline-block; font-size: 32px;">Best seller in </p><input class="input" id="month" placeholder="Month" style="margin-left: 5px;width: 73px;margin-bottom: 30px; text-align: center; padding-left: 0" value="' +
      m +
      '" onkeyup="reDrawChart()"></div>' +
      result;
    document.getElementById("center").innerHTML = result;
    drawChart();
  } else document.getElementById("center").innerHTML = result;
}
...
function saleAccordingToMonth() {
  ...
  for (var i = 0; i < save_order.length; i++) {
    ...
    if (order_month == m) save_order_2.push(save_order[i]);
  }
  console.log("test");
...
function isExitInTop10(product_id, top10) {
  for (var i = 0; i < top10.length; i++)
    if (product_id == top10[i].id) return 1;
  return 0;
}

function getTop10(flag) {
  var bestSeller;
  if (flag == true) bestSeller = saleAccordingToMonth();
  else bestSeller = saleAccordingToMonth2();
  var top10 = [];
  for (var i = 0; i < bestSeller.length; i++) {
    for (var j = 0; j < bestSeller[i].order.length; j++)
      if (isExitInTop10(bestSeller[i].order[j][0], top10) == 0)
        top10.push({
          id: bestSeller[i].order[j][0],
          Sold: getTotal(bestSeller[i].order[j][0], bestSeller),
        });
  }
...
function drawChart(flag = true) {
  if (localStorage["save_order"] != null) {
    ...
    if (bestSeller.length == 0) {
      ...
      document.getElementById("monthOfChart").style.marginTop = "25px";
    } else document.getElementById("myChart").style.display = "block";
    var label = [];
    var sale = [];
    var length;
    if (bestSeller.length > 10) length = 10;
    else length = bestSeller.length;
...
function blockAcc(name) {
  if (confirm("Bạn có muốn khóa tài khoản của user này không ?")) {
    name = name.slice(0, -1);
...
function getButton(user) {
  if (user.isBlocked == 1) {
    ...
    return (
      ...
      user.name +
      "U" +
      '" class="btn2" style="display: inline-block" onclick="unblockAcc(this.id)">Unblock</button>' +
      '<button id="' +
      user.name +
      "B" +
      '" class="btn2" style="display: none" onclick="blockAcc(this.id)">Block</button>'
    );
  }
  return (
    '<button id="' +
    user.name +
    "U" +
    '" class="btn2" style="display: none" onclick="unblockAcc(this.id)">Unblock</button>' +
    '<button id="' +
    user.name +
    "B" +
    '" class="btn2" style="display: inline-block" onclick="blockAcc(this.id)">Block</button>'
  );
}
...
function resetButton(user) {
  if (user.resetPW == 1)
    return (
      '<button id="' +
      user.name +
      '" class="btn2" style="display: inline-block" onclick="resetPw(this.id)">Reset</button>'
    );
  return " ";
}
    ...
    for (var j = 0; j < stored_accounts.length; j++) {
      if (stored_accounts[j].name.toLowerCase().indexOf(input) != -1) {
        temp +=
          ...
          '</h4><p style="margin-top: -21px; margin-bottom: -10px">Phone number: ' +
          stored_accounts[j].phoneNumber +
          '</p><p style="margin-bottom: -10px">Address: ' +
          stored_accounts[j].address +
          '</p><p style="margin-bottom: -10px">Email: ' +
          stored_accounts[j].email +
          '</p><p id="AccState' +
          stored_accounts[j].name +
          '">State: ' +
          isBlocked(stored_accounts[j].isBlocked) +
          "</p>" +
          getButton(stored_accounts[j]) +
          '<button id="' +
          stored_accounts[j].name +
          '"class="btn2" onclick="deleteAcc(this.id)">Delete</button>' +
          '<button id="' +
          stored_accounts[j].name +
          '"class="btn2" onclick="openForm3(this.id)">Edit</button>' +
          resetButton(stored_accounts[j]) +
          "</div>";
        c += 1;
...
function searchById() {
  ...
  var flag = false;
  if (input === "") document.getElementById("search_result").innerHTML = result;
  else {
    var flag = false;
    for (var j = 0; j < product_info.length; j++) {
      if (product_info[j].id.toLowerCase().indexOf(input) != -1) {
        temp +=
          ...
          '"><h4 style="font-size: 20px; margin-top:2%">' +
          product_info[j].name +
          '</h4><p style="margin-top: -21px; margin-bottom: -10px">Price: ' +
          product_info[j].price.toLocaleString("it-IT", {
            style: "currency",
            currency: "VND",
          }) +
          ...
          '<button class="btn" id="' +
          product_info[j].id +
          '" onclick="deleteProduct(this.id)">Delete</button><button id="' +
          product_info[j].id +
          '"class="btn" onclick="editInfo(this.id)">Edit</button></div>';
        c += 1;
        if (c == 4 || j == product_info.length - 1) {
          temp = '<div class="row2">' + temp + "</div>";
          result += temp;
...
function saveChanges(id) {
  ...
  for (var i = 0; i < product_info.length; i++)
    if (id == product_info[i].id) {
      if (product_name !== "") product_info[i].name = product_name;
      if (price !== "") product_info[i].price = parseInt(price);
      if (quantity !== "") product_info[i].quantity = parseInt(quantity);
      if (image !== "") {
        image = "../image/" + image.substr(12, image.length - 12);
...
function getInfo1() {
  ...
  if (flag != 0) {
    ...
    if (
      ...
      if (
        text1 == "Admin" &&
        localStorage.getItem("current_admin") == admins[0]
      ) {
        ...
        alert(
          "This action is beyond your authority, the account you just added is a normal user!"
        );
      }
      closeAddForm();
      openManageAccForm();
...
function getInfo2() {
  ...
  for (var i = 0; i < stored_accounts.length; i++)
    if (
      userName == stored_accounts[i].name &&
      current_name != stored_accounts[i].name
    ) {
      alert("This user name has been used.");
      flag = 0;
      break;
  ...
  if (flag == 1) {
    for (var i = 0; i < stored_accounts.length; i++)
      if (stored_accounts[i].name == current_name) {
        var admins = JSON.parse(localStorage.getItem("admins"));
        if (
          (current_name == admins[0] &&
            localStorage.getItem("current_admin") == admins[0]) ||
          checkAdmin(current_name) != 1 ||
          (current_name != admins[0] &&
            localStorage.getItem("current_admin") == admins[0])
        ) {
    ...
    if (flag == 1) {
      ...
      if (
        ...
        text1 != "Admin" &&
        userName != localStorage.getItem("current_admin") &&
        userName != admins[0]
      )
...
function deleteAcc(id) {
  ...
  for (var i = 0; i < stored_accounts.length; i++) {
    if (id == stored_accounts[i].name) {
      if (confirm("Bạn có muốn xóa tài khoản của user này không ?")) {
        stored_accounts.splice(i, 1);
        if (checkAdmin(id) == 1) removeAdmin(id);
      }
      break;
    }
  }
